...
def computeMatches(gtData, data, sessionId, recordingId, systemId):
    ...
    for gtCar in gtData["cars"]:
        ...
        filtered = sorted(filtered, key=lambda i: abs(i["timeIntersectionLast"]-gtTimeIntersection))
...
def prefilterData(data, gtData):
    fps = gtData["fps"]
    maxLineIntersection = max(list(map(lambda i: i["intersections"][-1]["videoTime"], gtData["cars"])))
...
def getCarTimeAndSpatialIntersection(line, posX, posY, frames, around=6):
    ...
    points = sorted(points, key=lambda i: pointToLineDistance(i[0], line))
    getItems = lambda getter: list(map(getter, points[:min(around,len(points))]))
    
    print(f"posX {(posX)}")
    print(f"posY {(posY)}")
    print(f"[0] {getItems(lambda i: i[0][0])}")
    print(f"[1] {getItems(lambda i: i[0][1])}")
...
def computeFalsePositives(gtData, matches, data):
    maxLineIntersection = max(list(map(lambda i: i["intersections"][-1]["videoTime"], gtData["cars"])))
    minLineIntersection = min(list(map(lambda i: i["intersections"][-1]["videoTime"], gtData["cars"])))
...
def showFalsePositives(systemsData):
    ...
    for k in RUN_FOR_SYSTEMS:
        data = systemsData[k]
        totalFalsePositives = sum(list(map(lambda i: i["falsePositives"], iter(data.values()))))
...
def showRecalls(systemsData):
    ...
    for k in RUN_FOR_SYSTEMS:
        data = systemsData[k]
        totalFalsePositives = np.mean(list(map(lambda i: computeRecall(i["matches"]), iter(data.values()))))
        tabulateData.append((labelConversion(k), totalFalsePositives))
...
def showErrorCumHistogram(data, prefix, unit, xlimMax = None):
    maxError = max(reduce(operator.add, iter(data.values())))
    ...
    if not os.path.exists(RESULTS_CACHE_FILE) or args.recomputeCache:
        ...
        for system in RUN_FOR_SYSTEMS:
            if system not in systemsData:
                ...
                sys.exit(1)
            print(set(systemsData[system].keys()))
